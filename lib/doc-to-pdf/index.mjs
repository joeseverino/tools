...
function documentProvenance(input) {
  const inputDir = path.dirname(input);
  const root = gitOutput(inputDir, 'rev-parse', '--show-toplevel');
  if (!root) return { label: path.basename(input), url: null };
  ...
  if (!repository || relative.startsWith('../')) {
    return { label: path.basename(input), url: null };
  }
  ...
  const label = `${repository.owner}/${repository.repo}/${relative}`;
  const url = branch
    ? `https://github.com/${repository.owner}/${repository.repo}/blob/${branch}/${relative}`
    : null;
  return { label, url };
}
...
const provenance = documentProvenance(input);
