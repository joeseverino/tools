...
import {
  ...
  svgDataUrl,
  htmlText,
  fileDataUrl as engineFileDataUrl,
  findChromium,
  vendoredFont,
  printHtmlToPdf,
} from '../pdf-engine/index.mjs';
...
function fileDataUrl(file, mediaType, label = 'file') {
  try {
    return engineFileDataUrl(file, mediaType);
  } catch {
    die(`${label} not found: ${file}`);
  }
}
...
function githubDocumentContext(input) {
  const inputDir = path.dirname(input);
  const root = gitOutput(inputDir, 'rev-parse', '--show-toplevel');
  if (!root) return null;

  const repository = githubRepository(gitOutput(root, 'remote', 'get-url', 'origin'));
  const relative = path.relative(
    fs.realpathSync(root),
    fs.realpathSync(input),
  ).split(path.sep).join('/');
  if (!repository || relative.startsWith('../')) return null;
...
function githubUrl(context, target, suffix = '') {
  const canonicalTarget = fs.existsSync(target) ? fs.realpathSync(target) : target;
  const relative = path.relative(context.root, canonicalTarget).split(path.sep).join('/');
  ...
  return `https://github.com/${context.owner}/${context.repo}/${kind}/${encodeURIComponent(context.branch)}/${encodedPath}${suffix}`;
}

function githubLink(context, input, href) {
  if (!context || !href || href.startsWith('#') || href.startsWith('//')) return href;
  ...
  const target = path.resolve(
    decodedPath.startsWith('/') ? context.root : path.dirname(context.input || input),
    decodedPath.replace(/^\/+/, ''),
  );
  return githubUrl(context, target, suffix) || href;
}

function documentProvenance(input, context) {
  if (!context) return { label: path.basename(input), url: null };

  return {
    label: `${context.owner}/${context.repo}/${context.relative}`,
    url: githubUrl(context, input),
  };
}
...
const githubContext = githubDocumentContext(input);
...
md.renderer.rules.link_open = (tokens, index, options, env, self) => {
  ...
  if (hrefIndex >= 0) {
    tokens[index].attrs[hrefIndex][1] = githubLink(
      githubContext,
      input,
...
const provenanceContent = provenance.url
  ? `<a href="${htmlText(provenance.url)}">${htmlText(provenance.label)}</a>`
  : htmlText(provenance.label);
const brandedTitle = `<h1 class="document-title"><span class="document-mark">${brandMark}</span><span>${titleContent}</span><span class="document-provenance">${provenanceContent}</span></h1>`;
body = titleMatch
...
<style>
  ...
  * { box-sizing: border-box; }
  body { font: 15px/1.55 "Inter", sans-serif; color: var(--brand-ink); background: var(--brand-paper); margin: 0; }
  main { padding-inline: 2mm; }
  ...
  .document-provenance { margin-left: auto; align-self: flex-end; margin-bottom: 4px; font-size: 11px; font-weight: normal; color: color-mix(in srgb, var(--brand-ink) 58%, var(--brand-paper)); }
  .document-provenance a { color: inherit; text-decoration: none; }
  ...
  img[src*="checks-engine.png"] { max-height: 65vh; width: auto !important; }
  img.mermaid-diagram { width: 100%; page-break-inside: avoid; }
  table { border-collapse: collapse; width: 100%; max-width: 100%; table-layout: fixed; margin: 1em 0; font-size: 12px; page-break-inside: auto; break-inside: auto; }
  thead { display: table-header-group; }
  tfoot { display: table-footer-group; }
  tr { page-break-inside: avoid; break-inside: avoid; }
  th, td { min-width: 0; border: 1px solid color-mix(in srgb, var(--brand-ink) 20%, var(--brand-paper)); padding: 5px 7px; text-align: left; vertical-align: top; overflow-wrap: anywhere; word-break: break-word; }
  th code, td code { white-space: normal; overflow-wrap: anywhere; }
...
try {
  printHtmlToPdf({
    ...
    html,
    output,
    tmpPrefix: 'doc-to-pdf',
    keepHtml,
    onKeepHtml: (tmpHtml) => console.log(`doc-to-pdf: kept HTML at ${tmpHtml}`),
  });
