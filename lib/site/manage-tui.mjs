...
import {
  ...
  truncate, lineEditor, fitFrame, previousBoundary, nextBoundary,
  enterAlt, leaveAlt, createTitleSetter, createInputPump, NAMED_KEYS,
} from '../tui.mjs';
import { svmc } from '../sdk/svmc.mjs';
import { run } from '../sdk/process.mjs';
...
function configureFields(contentContract) {
  const fields = contentContract?.contract?.collections?.writeups?.fields;
  if (!fields) fail('writeup dashboard did not include a content contract');
...
function isPortOpen(port) {
  return run('lsof', ['-t', '-i', `tcp:${port}`], { stdio: 'ignore' }).ok;
}

function stopPort(port) {
  const result = run('lsof', ['-t', '-i', `tcp:${port}`]);
  if (!result.ok) return;
...
function loadSiteStatus() {
  ...
  try {
    const branch = run('git', ['-C', SITE_HOME, 'rev-parse', '--abbrev-ref', 'HEAD']);
    if (!branch.ok) throw new Error(branch.error);
    status.gitBranch = branch.stdout.trim();
    const git = run('git', ['-C', SITE_HOME, 'status', '--porcelain']);
  ...
  try {
    const git = run('git', ['-C', SITE_HOME, 'log', '-1', '--format=%h%x09%s%x09%cr']);
    if (!git.ok) throw new Error(git.error);
  ...
  try {
    const git = run('git', ['-C', SITE_HOME, 'rev-list', '--left-right', '--count', 'HEAD...@{u}']);
    if (!git.ok) throw new Error(git.error);
  ...
  try {
    const site = run(SITE_BIN, ['check-security']);
  ...
  try {
    const curl = run('curl', ['-s', '-o', '/dev/null', '-m', '2', '-w', '%{http_code}', LIVE_URL]);
...
function load() {
  ...
  const contractFields = configureFields(res.json.content_contract);
...
// ---- rendering --------------------------------------------------------------
// Width/clip/line-editor/frame-fit helpers live in ../tui.mjs (the shared TUI
// library); only the terminal-size getters here are tool-specific (they read
// MANAGE_TUI_* test overrides).
...
function apply(model) {
  ...
  const plan = { updates, source_fingerprint: model.sourceFingerprint };
  if (featuredChanged) {
...
// Alt-screen + window-title polish bar lives in ../tui.mjs.
    ...
    case 'detail':
      switch (key) {
        ...
        case '\r': {
          ...
          model.mode = 'edit';
          model.input = String(fieldValue(model.items[model.cursor], f) || '');
          model.inputCursor = model.input.length;
          break;
        }
...
// The escape-sequence input pump (split arrows, bracketed paste, esc timeout)
// lives in ../tui.mjs; here we just bind it to this tool's key + paste handlers.
