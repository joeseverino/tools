...
const KEYMAP = [
  ...
  { keys: 'd', hint: 'view the diff in a scrollable pane (git diff HEAD)', foot: 'd diff' },
...
function workflowActions(r) {
  ...
  actions.push({
    ...
    why: 'the working-tree diff against HEAD, in a scrollable pane (no shell drop)',
    effect: 'read',
    overlay: true,
  });
  actions.push({
    ...
    why: 'the repo status, in a scrollable pane (no shell drop)',
    effect: 'read',
    overlay: true,
  });
...
// A scrollable read-only pane (diff / git status) over the dashboard — same
// alt-screen, same raw mode, so opening and closing it never disturbs input.
function overlayFrame(model) {
  const cols = terminalColumns();
  const ov = model.overlay;
  const width = Math.max(40, cols - 4);
  const body = overlayBody();
  const maxTop = Math.max(0, ov.lines.length - body);
  const top = Math.min(Math.max(0, ov.top), maxTop);
  const out = [
    '',
    `  ${BOLD}${ov.title}${RESET}${DIM} — ${ov.lines.length} line${ov.lines.length === 1 ? '' : 's'}${RESET}`,
    `  ${DIM}${truncate(ov.cmd, width)}${RESET}`,
    `  ${'─'.repeat(width)}`,
  ];
  const slice = ov.lines.slice(top, top + body);
  for (let i = 0; i < body; i += 1) out.push(`  ${truncate(slice[i] || '', width)}`);
  out.push(`  ${DIM}${'─'.repeat(width)}${RESET}`);
  const pos = maxTop > 0 ? ` · ${top + 1}-${Math.min(top + body, ov.lines.length)}/${ov.lines.length}` : '';
  out.push(`  ${DIM}↑/↓ · j/k scroll · space/b page · q/esc close${pos}${RESET}`);
  return out.join('\n');
}

function frame(model) {
  if (model.loading || model.error) return statusFrame(model);
  if (model.overlay) return overlayFrame(model);
...
// Run a read-only inspection (diff / git status) and stash its output as a
// scrollable overlay — the TUI never leaves the alt-screen or raw mode, so there
// is no shell drop, no "press return", no echo, and no fleet rescan on return.
// Synchronous + hermetic (just runs the action's own command), so replay/smoke
// exercise it too.
function showOutput(model, action) {
  const repo = selectedRepo(model);
...
function overlayBody() {
  return Math.max(3, terminalRows() - 7);
}
...
async function executeAction(model, givenAction = null) {
  ...
  if (!action) return;
  // Inspections render in a scrollable in-TUI overlay — no shell drop, no echo,
  // no rescan. Handled before the replay short-circuit so it stays deterministic.
  if (action.overlay) { showOutput(model, action); return; }
  if (REPLAY || SMOKE) {
...
async function handleKey(model, key) {
  ...
  if (model.overlay) {
    const body = overlayBody();
    const maxTop = Math.max(0, model.overlay.lines.length - body);
    switch (key) {
      case '\x1b[B': case 'j': model.overlay.top = Math.min(model.overlay.top + 1, maxTop); break;
      case '\x1b[A': case 'k': model.overlay.top = Math.max(0, model.overlay.top - 1); break;
      case ' ': case '\x1b[6~': model.overlay.top = Math.min(model.overlay.top + body, maxTop); break;
      case 'b': case '\x1b[5~': model.overlay.top = Math.max(0, model.overlay.top - body); break;
      case 'g': model.overlay.top = 0; break;
      case 'G': model.overlay.top = maxTop; break;
      case 'q': case '\x1b': case '\r': model.overlay = null; break;
