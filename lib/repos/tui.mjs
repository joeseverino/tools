...
// they can never drift (the house rule: interactive UIs self-document their keys).
const KEYMAP = [
  ['↑/↓ · j/k', 'move within the focused pane'],
  ['←/→ · h/l', 'switch workflow view'],
  ['Tab', 'switch between the repo list and its actions'],
  ['Enter', 'focus actions, then copy the selected command'],
  ['c', 'copy the selected action command'],
  ['x', 'run the selected action (commit/push/merge/resync)'],
  ['s', 'open a login shell in the selected repo'],
  ['d', 'view the diff (git diff HEAD, paged)'],
  ['m', 'land — merge the open PR and delete its branch'],
  ['o', 'open the repo on GitHub (no shell drop)'],
  ['A', 'resync the whole fleet (safe: skips dirty/diverged)'],
  ['p', 'copy the repo path'],
  ['g', 'copy a git status command for the repo'],
  ['/', 'filter by name/path/branch/remote/PR'],
  ['r', 'reload the local snapshot'],
  ['F', 'full refresh: git fetch + PR/CI state'],
  ['?', 'toggle this help'],
  ['q · Esc', 'quit'],
];
...
function frame(model) {
  ...
  const out = [];
  out.push('');
  const prNote = model.prsLoading ? ` · ${DIM}PRs…${RESET}${DIM}` : c.pr ? ` · ${c.pr} PR` : '';
  ...
  if (model.filtering) {
    ...
    out.push(`  ${DIM}↑/↓ move · ←/→ view · Tab pane · Enter copy · x run · d diff · m land · ? help · q quit${RESET}`);
  }
  return out.join('\n');
}
...
function helpFrame(model) {
  ...
  for (const [keys, desc] of KEYMAP) {
    out.push(`  ${CYAN}${padEndAnsi(keys, 12)}${RESET} ${DIM}${truncate(desc, Math.max(10, cols - 18))}${RESET}`);
  }
  out.push('');
...
async function handleKey(model, key) {
  ...
  switch (key) {
    ...
    case 'd': await runNamedAction(model, 'diff', 'nothing to diff'); break;
