...
function normalizeRepo(raw) {
  return {
    ...
    needsShip: raw.needs_ship === undefined ? null : !!raw.needs_ship,
    needsResync: raw.needs_resync === undefined ? null : !!raw.needs_resync,
    needsAttention: raw.needs_attention === undefined ? null : !!raw.needs_attention,
...
function needsShip(r) {
  if (r.needsShip !== null) return r.needsShip;
  return dirtyCount(r) > 0 || r.ahead > 0 || !r.hasRemote || !r.upstream;
}

function needsResync(r) {
  if (r.needsResync !== null) return r.needsResync;
...
function workflowActions(r) {
  ...
  if (url) {
    actions.push({
      ...
      why: 'open the repository on GitHub in Safari',
...
function frame(model) {
  ...
  if (model.filtering) {
    ...
    out.push(`  ${DIM}↑/↓ repo · ←/→ view · Tab actions · Enter copy · x run · s shell · o GitHub · / search · r refresh · q quit${RESET}`);
  }
  return out.join('\n');
}
