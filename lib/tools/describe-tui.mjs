...
import {
  ...
  truncate, displayWidth, wrapText, lineEditor, fitFrame, previousBoundary, nextBoundary,
...
// A risk chip from the v3 effect triple. A plain read with no reach renders
...
function rightPane(model, height, width) {
  ...
  for (const line of wrapText(tool.description, width)) {
  ...
  if (tool.globalOptions.length) {
    ...
    for (const o of tool.globalOptions) {
      pushRow(lines, 2, `${CYAN}${padEndAnsi(argLabel(o), 22)}${RESET}`, 22, o.help, width);
    }
    lines.push('');
  }
  ...
  if (tool.positionals.length) {
    ...
    for (const a of tool.positionals) {
      pushRow(lines, 2, padEndAnsi(argLabel(a), 22), 22, a.help, width);
    }
    lines.push('');
  }
  ...
  if (tool.commands.length) {
    ...
    tool.commands.forEach((cmd, idx) => {
      ...
      // The selected command shows its full summary (wrapped) so nothing is
      // hidden behind an ellipsis; the rest stay one scannable truncated line.
      if (selected) {
        pushRow(lines, 0, `${pointer} ${name}`, 20, cmd.summary, width);
        const tag = effectTag(cmd);
        if (tag) lines.push(`${' '.repeat(6)}${tag}`);
        for (const a of cmd.args || []) {
          pushRow(lines, 6, `${CYAN}${padEndAnsi(argLabel(a), 20)}${RESET}`, 20, a.help, width);
        }
      } else {
        const summary = truncate(cmd.summary || '', Math.max(8, width - 21));
        lines.push(`${pointer} ${name} ${DIM}${summary}${RESET}`);
      }
    });
  } else {
