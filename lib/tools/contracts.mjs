...
export function inspectContracts({ check = false, scope = 'fleet', id = null } = {}) {
  ...
  const contracts = contractsSelected.map((contract) => ({
    ...contract,
...
export function deriveProjections({ id = null, all = false, scope = 'local', go = false } = {}) {
  if (!['local', 'fleet'].includes(scope)) throw new Error(`unknown derive scope: ${scope}`);
...
function main() {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (!args[index].startsWith('-') && !id) id = args[index];
    else throw new Error(`unknown option or extra id: ${args[index]}`);
  }
