...
function waitForReturn(label) {
  return new Promise((resolve) => {
    ...
    // Raw, not cooked: a single keypress dismisses, with no echo and no line
    // buffering. Cooked mode here was the source of arrow-key escape codes
    // (^[[A…) getting echoed onto the screen while the prompt waited for Enter.
...
export async function runForegroundAction(action, label = 'tui') {
  ...
  enterAlt();
  // Force-reapply raw mode. After a child that drove the TTY (a pager), Node's
  // cached raw flag can be stale, so a bare setRawMode(true) is a no-op that
  // leaves the terminal cooked — keys then echo instead of reaching the TUI. The
  // false→true toggle guarantees the raw ioctl actually runs.
