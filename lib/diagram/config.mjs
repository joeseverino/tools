...
// Explicit override wins (and fails loud if broken); the legacy brand-kit
// path is honored when present; otherwise fall back to pdf-engine's vendored
// Inter — severino-brand stopped shipping fonts when the branding engine
// adopted a bundled default, so a bare kit is the normal case.
...
const brandFont = path.resolve(
  process.env.DIAGRAM_FONT
    || (fs.existsSync(brandKitFont)
      ? brandKitFont
      : path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'pdf-engine', 'fonts', 'inter-variable-latin.woff2')),
);
