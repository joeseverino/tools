...
// A plugged section's rows carry its own name as the surface label.
const sectionSurface = (name) => ({ label: String(name).toUpperCase().slice(0, 8), color: MAGENTA });
...
function buildItems(digest) {
  ...
  for (const name of repos.stale || []) items.push({
    surface: 'repo', sev: 85, subject: `${name} on a stale branch`,
    why: 'committed work behind main with no PR — shipping here re-inherits merged work; replay onto a fresh branch off main',
    action: { label: 'rebranch', cmd: `ship ${shellQuote(name)} --rebranch --check --go`, effect: 'remote_write + network' },
  });
  ...
  // Plugged sections contribute pre-ranked rows; the emitter owns severity,
  // wording, and the action — this queue only merges and sorts.
  for (const s of digest.sections || []) {
    for (const q of s.queue || []) items.push({
      surface: s.name, sectionSurface: true,
      sev: Number.isFinite(q.sev) ? q.sev : 30,
      subject: q.subject || '', why: q.why || '',
      action: q.action
        ? { label: q.action.label || 'copy', cmd: q.action.cmd || '', effect: q.action.effect || 'read', copyOnly: !!q.action.copy_only }
        : undefined,
    });
  }
...
function frame(model) {
  ...
  const rowsOut = model.items.map((it, idx) => {
    ...
    const s = it.sectionSurface ? sectionSurface(it.surface)
      : SURFACE[it.surface] || { label: it.surface, color: DIM };
